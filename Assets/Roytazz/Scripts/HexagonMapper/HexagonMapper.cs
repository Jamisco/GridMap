...
namespace Roytazz.HexMesh
{
    ...
    public class HexagonMapper
    {
        ...
        public float HorizontalOffset {
            get {
                switch (Type) {
                    ...
                    default:
                        throw new Exception($"Can't get {nameof(HorizontalOffset)} with unexpected {nameof(Type)}");
                }
            }
        }
            ...
            get {
                switch (Type) {
                    ...
                    case HexagonalGridType.FlatEven:
                        return InscribedRadius * 2.0f;
                    default:
                        throw new Exception($"Can't get {nameof(VerticalOffset)} with unexpected {nameof(Type)}");
                };
            }
        }
            ...
            get {
                switch (Type) {
                    ...
                    case HexagonalGridType.FlatEven:
                        return 30.0f;
                    default:
                        throw new Exception($"Can't get {nameof(AngleToFirstNeighbor)} with unexpected {nameof(Type)}");
                };
            }
        }
        ...
        public HexagonMapper(HexagonalGridType type, float radius) {
            Type = type;
        ...
        public Offset ToOffset(Cubic coord) {
            switch (Type) {
                ...
                default:
                    throw new Exception($"{nameof(ToOffset)} failed with unexpected {nameof(Type)}: {nameof(coord)} {coord}");
            }
        }
        ...
        public Cubic ToCubic(Offset coord) {
            switch (Type) {
                ...
                default:
                    throw new Exception($"{nameof(ToCubic)} failed with unexpected {nameof(Type)}: ${nameof(coord)} {coord}");
            }
        }
        ...
        public Cubic ToCubic(float x, float y) {
            switch (Type) {
                ...
                default:
                    throw new Exception($"{nameof(ToCubic)} failed with unexpected {nameof(Type)}: {nameof(x)} {x}, {nameof(y)}, {y}");
            }
        }
        ...
        /// </summary>
        public Vector2 ToPoint2v(Offset coord) {
            (float X, float Y) = ToPoint2(coord);
        ...
        public (float X, float Y) ToPoint2(Axial coord) {
            switch (Type) {
                ...
                default:
                    throw new Exception($"{nameof(ToPoint2)} failed with unexpected {nameof(Type)}:, {nameof(coord)} {coord}");
            }
        }
        ...
        /// </summary>
        public Vector2 GetCornerPointv(Offset coord, int edge) {
            var (X, Y) = GetCornerPoint(coord, edge);
            return new Vector2(X, Y);
        }
        ...
        private (float X, float Y) GetCornerPoint<T>(T coord, int edge, Func<T, (float X, float Y)> toPoint)
            ...
            var (X, Y) = toPoint(coord);
            var angleRad = PI / 180 * angleDeg;
            var x = (float)(X + DescribedRadius * Cos(angleRad));
            var y = (float)(Y + DescribedRadius * Sin(angleRad));
            return (x, y);
        }
        ...
        private byte GetNeighborIndex<T>(T center, T neighbor, Func<T, IEnumerable<T>> getNeighbors)
            ...
            throw new Exception($"Can't find bypass index: {nameof(center)} {center}, {nameof(neighbor)} {neighbor}");
        }
        ...
        private (float x, float y) GetPointBetweenTwoNeighbours<T>(T coord1, T coord2, Func<T, T, bool> isNeighbor, Func<T, (float X, float Y)> toPoint)
            ...
            if (!isNeighbor(coord1, coord2)) {
                throw new Exception($"Can't calculate point between not neighbors: {nameof(coord1)} {coord1}, {nameof(coord2)} {coord2}");
            }

            var (x1, y1) = toPoint(coord1);
            var (x2, y2) = toPoint(coord2);
        ...
        private IReadOnlyList<Offset> GetNeighborsOffsets(Offset coord) {
            switch (Type) {
                ...
                default:
                    throw new Exception($"{nameof(GetNeighborsOffsets)} failed with unexpected {nameof(Type)}: {nameof(coord)} {coord}");
            };
        }
        ...
        /// <returns></returns>
        public static int[] HexagonTriIndexes(int indexOffset)
            => new int[12] {
            4 + indexOffset, 3 + indexOffset, 2 + indexOffset,
            4 + indexOffset, 2 + indexOffset, 1 + indexOffset,
            4 + indexOffset, 1 + indexOffset, 5 + indexOffset,
            5 + indexOffset, 1 + indexOffset, 0 + indexOffset
        };
    }
